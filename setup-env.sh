...
symlink_folder_contents() {
    ...
    for plugin in "$target_dir"/*/; do
        ...
        ln -s $target_dir/$plugin $output_dir/$plugin
    done
}
...
# Software
ln -s $PATH_TO_REPO/software/ohmyzsh $OHMYZSHHOME
...
ln -s $PATH_TO_REPO/dotfiles/clangd/config.yaml $HOME/.config/clangd/config.yaml
